...
def run(master_server_ip, own_ip, crawler, logging_level, backup_port):
    ...
    server.start()
    try:
        threading.Thread(target=sendHeartBeatMessage, args=(
            master_server_ip, server, master, logger, crawler, logging_level, ), daemon=True).start()
    except Exception as e:
        print(str(e))
