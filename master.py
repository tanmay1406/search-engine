...
# aliasing keeps every existing thread.start_new_thread(...) call below unchanged
...
class Master(object):
    ...
    def __init__(self, db_name, ip, backup, logging_level):
        ...
        # keeps track of search queries and categories in location
        self.loc_count = LRUCache(maxsize=1000)
        # keeps track of categories whose loc_count >= THRESHOLD_COUNT
        self.cat_count = LRUCache(maxsize=1000)
        # own IP address
        self.ip = ip
        self.master_backup_ip = backup

        if db_name not in ['master', 'backup']:
            # keeps track of the time for which a word has not been queried from a replica
            self.wordIdletimes = {}
        ...
        if self.db in ['master', 'backup']:
            if location is not None and len(location) != 0 and location in replica_list:
                if location not in loc_count:
                    loc_count[location] = LRUCache(maxsize=1000)
                if search_term not in loc_count[location]:
                    loc_count[location][search_term] = 0
                loc_count[location][search_term] += 1

                if loc_count[location][search_term] >= THRESHOLD_COUNT:
                    if location not in cat_count:
                        cat_count[location] = set()
                    cat_count[location].add(search_term)
                    if len(cat_count[location]) >= THRESHOLD_CATEGORIES:
                        indices = list(cat_count[location])
                        data, indices_to_put = get_data_for_indices(
                            self.db, indices)

                        replica_ip, indices_present = query_metadatadb(
                            self.db, location, indices)
                        ...
                        if replica_ip is None or indices_present == False:  # replica needs to be created or updated
                            ...
                            if replica_ip is None:  # replica not present already
                                ...
                                for replica in replica_list[location]:
                                    self.logger.info(
                                        "Setting up the replica in "+location + " at " + replica)
                                    print("Setting up the replica in " +
                                          location + " at " + replica)
                                    replica_ip = replica

                                    channel = grpc.insecure_channel(replica_ip)
                                    stub = search_pb2_grpc.ReplicaCreationStub(
                                        channel)
                                    request = search_pb2.ReplicaRequest(
                                        data=data, master_ip=self.ip, create=True)
                                    break

                            elif replica_ip is not None and indices_present == False:  # replica present but indices not present
                                self.logger.info(
                                    "Adding indices to the replica in "+location + " at " + replica_ip)
                                print("Adding indices " + str(indices_to_put) +
                                      "to the replica in " + location + " at " + replica_ip)
                                channel = grpc.insecure_channel(replica_ip)
                                stub = search_pb2_grpc.ReplicaCreationStub(
                                    channel)
                                request = search_pb2.ReplicaRequest(
                                    data=data, master_ip=self.ip, create=False)

                            # create or update the replica
                            try:
                                print("Querying the replica")
                                response = stub.CreateReplica(
                                    request, timeout=10)
                                print(response)
                                ...
                                if self.db == 'master':
                                    ...
                                    self.initiate_2_phase_commit(
                                        replica_ip, location, indices_to_put)
                                else:  # backup
                                    # don't do anything since master has crashed
                                    add_to_metadatadb(
                                        self.db, replica_ip, location, indices_to_put)
                                ...
                                if isinstance(e, grpc.RpcError):
                                    if str(e.code()) == "StatusCode.DEADLINE_EXCEEDED":
                                        ...
                                        self.logger.error(
                                            "Deadline exceed - timeout before response received")
                            ...
                            self.logger.debug(
                                "Received query: " + search_term + "redirecting to replica " + replica_ip + " at " + location)
                        ...
                        try:
                            ...
                            print(str(e))
                            self.logger.error(
                                "Replica could not be contacted, falling back to master")
    ...
    def initiate_2_phase_commit(self, replica_ip, location, indices_to_put):
        ...
        try:
            ...
            request = search_pb2.CommitRequest(
                replica_ip=replica_ip, location=location, indices=indices_to_put)
            backup_vote = backup_stub.QueryToCommit(request, timeout=10)
            ...
            if isinstance(e, grpc.RpcError):
                ...
                logger.error("Backup not reachable due to " + str(e))
        ...
        if backup_vote is not None:
            if backup_vote.status == 1:
                ...
                while retries < MAX_RETRIES:
                    try:
                        backup_ack = backup_stub.CommitPhase(
                            request, timeout=10)
                        retries = MAX_RETRIES
    ...
    def Check(self, request, context):
        ...
        # NOTE (py3 port): must snapshot with list(...) before iterating, because the
        # loop body below calls .pop() on the same dict. In Python 2, .keys() returned
        # a real list so this was safe; in Python 3 it returns a live view, and mutating
        # the dict while iterating that view raises "RuntimeError: dictionary changed
        # size during iteration".
        for word in list(self.wordIdletimes.keys()):
    ...
    def UpdateReplica(self, request, context):
        self.logger.debug(
            "Received Update Request from master " + request.master_ip)
        print(request.data)
...
def updateReplicaAndBackup(master):
    while True:
        ...
        for replica_ip in replica_ips:
            ...
            request = search_pb2.ReplicaRequest(
                data=data, master_ip=master.ip, create=False)
            try:
                master.logger.info(
                    "Sending update message to replica " + replica_ip)
                response = stub.UpdateReplica(request, timeout=10)
                print(response)
                master.logger.info(
                    "Received " + str(response.status) + " from replica " + replica_ip)
                if response.status == 1:
                    master.logger.info(
                        "Changing is_new in " + master.db + "db")
                else:
                    master.logger.error("Replica db update failed")
            except Exception as e:
                print(str(e))
                master.logger.error(
                    "Replica " + replica_ip + " not reachable due to " + str(e))
        ...
        if master.db == 'master':
            ...
            if not len(indices_to_put) == 0:
                ...
                try:
                    master.logger.info(
                        "Sending update message to master backup " + master.master_backup_ip)
                    response = stub.UpdateReplica(request, timeout=10)
                    # print(response)
                    master.logger.info(
                        "Received " + str(response.status) + " from master backup " + master.master_backup_ip)
                    if response.status == 1:
                        master.logger.info(
                            "Changing is_new in " + master.db + "db")
                    ...
                    print("Here!")
                    master.logger.error(
                        "Master backup " + master.master_backup_ip + " not reachable due to " + str(e))
...
def heartbeatThread(db_name, master, replica_ip, location):
    ...
    try:
        ...
        master.logger.info("Received " + str(response.status) +
                           " from replica " + replica_ip)
    except Exception as e:
        print(str(e))
        if isinstance(e, grpc.RpcError):
            if str(e.code()) == "StatusCode.DEADLINE_EXCEEDED":
        ...
        if flag == 1:
            print("Setting up alternate replica at location ",
                  location, "at IP ", new_replica_ip)
            master.logger.info(
                "Setting up alternate replica in "+location + " at " + new_replica_ip)
            ...
            request = search_pb2.ReplicaRequest(
                data=data, master_ip=master.ip, create=True)
...
def sendHeartbeatsToReplicas(db_name, master):
    while True:
        for (replica_ip, location) in get_replica_ips_locs_from_metadatadb(db_name):
            threading.Thread(target=heartbeatThread, args=(
                db_name, master, replica_ip, location), daemon=True).start()
...
def serve(db_name, ip, backup, logging_level=logging.DEBUG, port='50051'):
    ...
    try:
        threading.Thread(target=updateReplicaAndBackup,
                         args=(master,), daemon=True).start()
    except Exception as e:
        print(str(e))
        master.logger.error("Cannot start new thread due to " + str(e))

    try:
        threading.Thread(target=sendHeartbeatsToReplicas,
                         args=(db_name, master,), daemon=True).start()
    except Exception as e:
        print(str(e))
        master.logger.error("Cannot start new thread due to " + str(e))
    ...
    def handle_shutdown(signum, frame):
        ...
        shutdown_event.set()
        master.logger.info("Shutting down server gracefully")
        server.stop(5)
        logging.shutdown()
